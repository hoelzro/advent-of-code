...
type ProgramInput interface {
	Read(program []int, ip int) (int, bool)
}
...
func RunProgram(input ProgramInput, output ProgramOutput, program []int, ip int) {
	relativeBase := 0
...
programLoop:
	for {
		...
		case 3:
			...
			value, ok := input.Read(program, ip)
			...
			ip += 2
		case 4:
			output.Write(getValue(&program, ip+1, modes[1], relativeBase))
			ip += 2
		case 5:
